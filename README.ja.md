...
| `/dronepattern reload` | データファイルからパターンを再読込（手編集をプラグイン全体リロード無しで反映） |
...
- **より大きい/精密な絵**は、データファイルを**直接編集**すれば**サイズ無制限**で作れます（`#`/`.` の行を並べるだけ。画像から変換して貼り付けるのも可）。
  - **正確なパス**: `oxide/data/DroneShow_Patterns.json`（Carbon は `carbon/data/DroneShow_Patterns.json`）。
  - **編集後は読み込みが必要**: `/dronepattern reload`（またはプラグインのリロード）。サーバー稼働中は自動では反映されません。
  - **JSONを壊さない**: プレーンテキストエディタで**半角のストレート引用符** `"` を使い、スマート/全角引用符は使わない。各パターンは**文字列（行）の配列**です。ファイルが壊れている場合はサーバーコンソールに警告を出し、**既存パターンは維持**します（無言で全消しはしません）。`/dronepattern reload` は読み込めた件数を表示するので、編集が正しく解析できたか確認できます。
- ただし**表示できるのは点灯ドット数 ≤ `Max drones per group`（既定2500）**まで（点灯1つ＝1機）。100×100でも「点が少ないスパースな絵」なら表示できますが、密に光らせる大きな絵は機数・サーバー負荷の上限に当たります。
...
> ⚠️ **誰が狙われるか**: 各ドローンは**アリーナ範囲内（中心 ± `Arena radius × 1.5`＝既定で約60m）にいる最寄りの接続プレイヤー**を狙います。**開始した人だけに限定されません。** 現状は**チーム／権限／管理者の除外フィルタが無い**ため、範囲内にいれば第三者でも攻撃対象になります。範囲外・死亡・スリープ中のプレイヤーは対象外です。
...
- 銃撃は通常のNPCと同じ挙動です。ガンナーは**あなたが見えている時だけ発砲**し（屋根・壁・コンテナが遮る）、各弾は**拡散コーン**内に散るため、**移動・遮蔽・距離で回避可能**です。`Gun - ...` のconfigキーで調整できます。
- ボスの大型化は `networkEntityScale` でクライアントへスケール同期（外部プラグイン不要）。

### ドロップ（報酬）
**プレイヤーが敵（またはボス）を撃墜**すると、撃墜地点に**アイテムが地面へ散らばります**。ドロップは**敵タイプごと**に自由に設定できます。

- config の `Loot - Tables per enemy type` を編集。キー: `Charger` / `Bomber` / `GunnerRapid` / `GunnerSniper` / `GunnerShotgun` / `Boss`。各項目は `Item shortname`（アイテム名）・`Min amount`・`Max amount`・`Chance (0-1)`・`Skin ID`・任意の `Custom name`。
- ドロップは**ゲーム進行中のプレイヤー撃墜時のみ**発生します（突撃の自爆・ゲーム停止時の撤去では出ません）。
- **ルート管理プラグイン連携（任意）**: `Loot - Also spawn container prefab for a loot plugin` にコンテナのプレハブを設定すると、撃墜時にそのコンテナ**も**スポーンし、**Loottable** などのルート管理プラグインが（そのプレハブ用に設定されていれば）中身を充填します。※Loottableへの直接API呼び出しではなく「相手が認識するコンテナを出す」方式です。空欄なら内蔵の散布テーブルのみ使用。
...
| `Max drones per group` | 2500 | グループ最大機数（長い文字ほど必要） |
...
| `Gun - Require line of sight to fire` | true | 屋根・壁越しに撃たない（見えている時だけ発砲） |
| `Gun - Min spread (deg)` | 1.5 | 最小拡散角。大きいほど避けやすい（必中させない） |
| `Gun - Target hitbox radius (m)` | 0.45 | プレイヤーの被弾サイズ。小さいほど避けやすい |
...
| `Loot - Enable drops` | true | プレイヤー撃墜時に敵がドロップ |
| `Loot - Scatter radius (m)` | 1.5 | 撃墜地点からアイテムが散らばる範囲 |
| `Loot - Tables per enemy type` | (タイプ別) | 敵/ボスごとのドロップ表（shortname・min/max・chance・skin・name） |
| `Loot - Also spawn container prefab for a loot plugin` | "" | 任意: Loottable等に充填させるコンテナも出す（空=無効） |

> **重要 — 設定値の反映方法（`.cs` ではなく JSON を編集）**
>
> プラグインはロード時に `DroneShow.json` を読み、**既存の値をそのまま保持**します（新キーだけ既定で補完し、既存値は上書きしません）。したがって:
> - **設定を変えるには**、**コンフィグファイル**の値を書き換えてプラグインをリロード:
>   - ファイル: `oxide/config/DroneShow.json`（Carbon は `carbon/config/DroneShow.json`）
>   - その後 `o.reload DroneShow`（Carbon は `c.reload DroneShow`）。
> - **`.cs` の既定値を書き換えても既存サーバーには反映されません** — コード既定はJSON新規生成時にしか使われません。
> - **全ての新既定を一括で採り込む**には、`DroneShow.json` を削除してリロード → コード既定から再生成されます（独自に変更した値は失われます）。
...
- **銃撃**: NPC相当のヒットスキャン。体の一点が見えている時だけ発砲（LOSゲート）、各弾は拡散コーンで散り、World/Terrain/Construction/Deployed で遮蔽され、体の当たり判定を線が通り遮蔽が無い時のみダメージ。遮蔽で防げ、距離・拡散で回避可能。
